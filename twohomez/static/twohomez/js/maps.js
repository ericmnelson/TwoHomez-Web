...
function heroMap(_latitude, _longitude, element, markerTarget,
  sidebarResultTarget, showMarkerLabels, mapDefaultZoom) {
  if (document.getElementById(element) != null) {
    ...
    function placeMarkers() {
      ...
      $(".result-item").on("mouseenter", function() {
        $(".map .marker[data-id=" + $(this).attr("data-id") +
      ...
      $(".result-item").each(function(i, result) {
        var marker;
        ...
        renderRichMarker(id, lat, lon, thumbnailImage, markerContent);
        //
        // // Only Address
        // else if (markers[i]["address"] && !markers[i]["latitude"] && !markers[
        //     i]
        //   ["longitude"]) {
      ...
      function renderRichMarker(id, lat, long, url, markerContent) {
        ...
        $.ajax({
          ...
          success: function(results) {
            ...
            setTimeout(function() {
              ...
              $(".item.infobox[data-id=" + id + "]").parent().addClass(
                "show");
            }, 10);

            google.maps.event.addListener(newMarkers[i].infobox,
              'closeclick',
              function() {
        ...
        return function(results, status) {
          if (status == google.maps.GeocoderStatus.OK) {
            ...
            google.maps.event.addListener(marker, 'click', (function(marker,
              i) {
              return function() {
                window.location.href = "/homes/" + $(this).data('id') +
                  "/details"
              }
            })(marker, i));
          } else {
      ...
      $(".marker").live("mouseenter", function() {
        ...
        $(".results-wrapper .results-content .result-item[data-id=" + id +
          "] a").addClass("hover-state");
      }).live("mouseleave", function() {
        var id = $(this).attr("data-id");
        $(".results-wrapper .results-content .result-item[data-id=" + id +
          "] a").removeClass("hover-state");
      });
      ...
      var clusterStyles = [{
        url: '/twohomez/static/twohomez/img/cluster.png',
        height: 36,
        width: 36
      }];
      ...
      function renderResults() {
        ...
        for (var i = 0; i < newMarkers.length; i++) {
          if (map.getBounds().contains(newMarkers[i].getPosition())) {
            ...
            visibleMarkersId.push($(newMarkers[i].content.firstChild).attr(
              "data-id"));
        ...
        $.ajax({
          ...
          success: function(results) {
            ...
            $(".results-wrapper .section-title h2 .results-number").html(
              visibleMarkersId.length); // show the number of results
            ...
            $(".result-item").children("a").on("click", function(e) {
              if (sidebarResultTarget == "sidebar") {
                ...
                e.preventDefault();
                openModal($(this).parent().attr("data-id"),
                  "modal_item.php");
              }

              $(lastClickedMarker).removeClass("active");

              $(".map .marker[data-id=" + $(this).parent().attr(
                "data-id") + "]").addClass("active");
              lastClickedMarker = $(".map .marker[data-id=" + $(this)
                .parent().attr("data-id") + "]");
            });
...
function simpleMap(_latitude, _longitude, element, markerDrag, place) {
  ...
  if (place) {
    ...
    function getCenterFromAddress() {
      return function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
          mapCenter = new google.maps.LatLng(results[0].geometry.location.lat(),
...
function autoComplete(map, marker) {
  if ($("#address-autocomplete").length) {
      ...
      if (place.address_components) {
        address = [
          (place.address_components[0] && place.address_components[0].short_name ||
            ...
            1].short_name || ''), (place.address_components[2] && place.address_components[
            2].short_name || '')
        ].join(' ');
      }
    });
    ...
    function success(position) {
      map.setCenter(new google.maps.LatLng(position.coords.latitude, position.coords
        .longitude));
